...
# Verifica la existencia de los archivos y llama a visualizar.py
def call_visualizar():
    ...
    if(not str(cant_muestras).isdigit()):
        ...
        showerror(title='ERROR',
                  message='La cantidad de muestras debe ser un número')
        return
    if len(encabezado_path):
        command_line = '%s visualizar.py "%s" %d "%s"'\
                       % (python, pregunta_path, cant_muestras,
                          encabezado_path)
    else:
...
# Verifica la existencia de los archivos y llama a generar.py
def call_generar():
    ...
    if not (os.path.isfile(student_list) or os.path.isdir(student_list)):
        print(student_list)
        showerror(title='ERROR',
                  message='La ruta de la lista de estudiantes no es valida')
        return
    if(not str(ind_rep).isdigit()):
        print(ind_rep)
        showerror(title='ERROR',
                  message='El indice de repetición debe ser un número')
        return
...
# Verifica la existencia de los archivos y llama a evaluar.py
def call_evaluar():
    ppp_path = entry_file_ppp.get()
    student_list = entry_file_estudents.get()
    ind_rep = indice_repeticion.get()
    ...
    if not os.path.isfile(resp_path):
        print(resp_path)
        showerror(title='ERROR',
                  message='El archivo .csv de respuestas no existe')
        return
    command_line = '%s evaluar.py "%s" "%s" "%s" %d'\
                   % (python, ppp_path, student_list, resp_path, ind_rep)
    print(command_line)
    os.system(command_line)
...
# permite controlar el numero de fila en la que se insertan elementos en
# la pantalla. Facilita agregar elementos en la parte superior.
row_number = 0

# ######################### Vista Previa ###############################
# Label indicar vista previa:
label_file_fun_vista_previa = tk.Label(master=frame_generar,
                                       text='Vista Previa')
...
label_file_pregunta_vp = tk.Label(
        master=frame_generar,
        text='1) Seleccionar archivo .tex de la pregunta')
...
label_cant_muestras = tk.Label(master=frame_generar,
                               text='2) Indique la cantidad de muestras')
...
entry_cant_muestras = tk.Entry(master=frame_generar, width=6,
                               textvariable=cantidad_muestras)
entry_cant_muestras.grid(column=2, row=row_number, padx=5, sticky='w')

row_number += 1
# Label para indicar archivo .tex del encabezado
label_file_encabezado_vp = tk.Label(
        master=frame_generar,
        text='3) Seleccionar archivo .tex del encabezado (opcional)')
label_file_encabezado_vp.grid(column=0, row=row_number, columnspan=3,
                              sticky='w')
...
button_vista_previa = tk.Button(master=frame_generar, text='Vista Previa',
                                command=call_visualizar)
...
label_file_ppp = tk.Label(master=frame_generar,
                          text='1) Seleccionar archivo .ppp')
...
label_file_estudents = tk.Label(
        master=frame_generar,
        text='2) Seleccionar lista de estudiantes (o directorio)')
...
label_file_ind_rep = tk.Label(
        master=frame_generar,
        text='3) Índice de repetición de examen (opcional)')
...
entry_file_ind_rep = tk.Entry(master=frame_generar, width=6,
                              textvariable=indice_repeticion)
entry_file_ind_rep.grid(column=2, row=row_number, padx=5, sticky='w')

row_number += 1
# Boton de generar
button_generar = tk.Button(master=frame_generar, text='Generar',
                           command=call_generar)
...
label_file_respuestas = tk.Label(
        master=frame_generar,
        text='4) Seleccionar archivo .csv de las respuestas')
...
row_number += 1
# Boton de evaluar
button_evaluar = tk.Button(master=frame_generar, text='Evaluar',
                           command=call_evaluar)
