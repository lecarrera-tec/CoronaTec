...
Puede utilizar \verb|error = math.inf| en caso de que deba aceptar todas las respuestas como correctas.
...
y realizar operaciones aritm\'eticas con ellos, las cuales se simplifican de manera autom\'atica. Se puede extraer luego su numerator y denominador mediante \verb|f.numerator| y \verb|f.denominator|. Aunque es un poco engorroso, para imprimir se utiliza \verb|fraccion(f.numerator, f.denominator)|.
...
\quad Simplifica e imprime utilizando \verb|dfrac|. De manera opcional se especifica si se obliga el signo \verb|+|; si el signo se imprime en el numerador o se imprime afuera; si se utiliza \verb|tfrac| en lugar de \verb|dfrac|, y si es un coeficiente, de manera que si da 1 o $-1$ no se imprima el valor.
...
\item \verb|txt.sumar(a: int, conSigno = True, arg = '')| \\
\quad Si $a=0$, no escribe nada. Si \verb|arg| es vac\'io, escribe el n\'umero. En caso contrario, escribe el coeficiente junto con el argumento.
